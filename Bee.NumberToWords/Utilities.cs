...
namespace Bee.NumberToWords
{
    internal static class Utilities
    {
        ...
        internal static string CapitalizeFirstLetter(this string words)
        {
            ...
            return words[0].ToString(CultureInfo.InvariantCulture).ToUpper(CultureInfo.InvariantCulture) + words.Substring(1);
        }
        ...
        internal static void ManageSuitableResources(out string[] ones, out string[] tens, out string[] scale, NumericWordsConversionOptions options)
        {
            switch (options.Culture)
            {
                ...
                case Culture.Arabic:
                    switch (options.OutputFormat)
                    {
                        case OutputFormat.Arabic:
                            ones = WordResources.OnesArabic;
                            tens = WordResources.TensArabic;
                            scale = WordResources.ScaleArabic;
                            break;
                        default:
                            throw new ArgumentOutOfRangeException();
                    }
